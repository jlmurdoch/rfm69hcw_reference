/*
 * Generic Radio Transmitter - RSL366
 *
...
// Switch OFF the first device on channel 1
byte state = 0;
byte dev = 1;
byte chan = 1;
...
void emitPulse(byte bit, int size) {
  // Section 3.2.2 - Async output is on DIO2  
  digitalWrite(RFM69HCW_G2_DIO2, bit);

  delayMicroseconds(size);
}

void emitData(uint16_t buf) {
  // Send four times
  for (int x=0; x<3; x++) {
    // from 12th to 1st bit (MSB>LSB)
    for (int i=11; i>=0; --i){
      // Leader pulse for each bit
      emitPulse(1, 400);
      emitPulse(0, 1340);

      // Respective pulse for binary bit
      emitPulse(1, 1340 - (((buf >> i) & 0x1) * 940));
      emitPulse(0, 400 + (((buf >> i) & 0x1) * 940));
    }
    // Sync pulse
    emitPulse(1, 400);
    emitPulse(0, 14340);
...
void setup() {
  ...
  // pinMode(RFM69HCW_G1_DIO1, OUTPUT);
